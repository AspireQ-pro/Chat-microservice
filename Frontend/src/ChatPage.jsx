...
import { useChatHandler, ROLE_COLOR } from '@/handler/chat'
...
function ChatPage() {
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
        ...
        {groupRooms.length > 0 && (
          <>
            ...
            <List disablePadding>
              {groupRooms.map((room) => (
                <GroupItem key={room.id} room={room} active={room.id === activeRoomId} onClick={() => handleSelectRoom(room)} />
              ))}
            </List>
          </>
        )}
    ...
    <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', minHeight: 0, minWidth: 0 }}>
      {activeContact ? (
        <>
          <Box sx={{ px: 2, py: 1.5, borderBottom: '1px solid', borderColor: 'divider',
            ...
            <Badge overlap="circular" anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
              badgeContent={<Circle sx={{ fontSize: 10, color: activeContact.online ? '#4caf50' : '#bdbdbd' }} />}>
          ...
          <Box sx={{ px: 2, py: 1.5, borderTop: '1px solid', borderColor: 'divider', bgcolor: 'background.paper' }}>
            {selectedFile && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1, px: 0.5 }}>
                ...
                <Typography variant="caption" sx={{ color: 'error.main', cursor: 'pointer', fontWeight: 600 }}
                  onClick={handleRemoveFile}>✕</Typography>
              </Box>
            )}
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column',
          alignItems: 'center', justifyContent: 'center', bgcolor: '#fafafa', gap: 1 }}>
  ...
  return (
    <Paper elevation={0} sx={{
      ...
      border: '1px solid', borderColor: 'divider', borderRadius: 0, overflow: 'hidden',
    }}>
      {isMobile ? (showPanel ? ContactPanel : ChatWindow) : <>{ContactPanel}{ChatWindow}</>}

      <CreateGroupDialog
        open={groupDialogOpen} onClose={closeGroupDialog}
        contacts={contacts} groupName={groupName} setGroupName={setGroupName}
        groupMemberIds={groupMemberIds} toggleGroupMember={toggleGroupMember}
        groupCreating={groupCreating} groupError={groupError} handleCreateGroup={handleCreateGroup}
      />
      <GroupMembersDialog
        open={membersDialogOpen} onClose={() => setMembersDialogOpen(false)} room={activeRoom}
      />
    </Paper>
  )
}
